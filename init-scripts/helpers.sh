...
wait_for_all_dbs() {
  ...
  wait_for_db "http://$DB_USER:$DB_PASSWORD_MASTER@$DB_HOST_MASTER:$DB_PORT_MASTER"
  wait_for_db "http://$DB_USER:$DB_PASSWORD_CAR1@$DB_HOST_CAR1:$DB_PORT_CAR1"
  wait_for_db "http://$DB_USER:$DB_PASSWORD_CAR2@$DB_HOST_CAR2:$DB_PORT_CAR2"
}
...
setup_users_databases() {
  ...
  check_and_create_users_db "$DB_HOST_MASTER" "$DB_PORT_MASTER" "$DB_USER" "$DB_PASSWORD_MASTER"
  check_and_create_users_db "$DB_HOST_CAR1" "$DB_PORT_CAR1" "$DB_USER" "$DB_PASSWORD_CAR1"
  check_and_create_users_db "$DB_HOST_CAR2" "$DB_PORT_CAR2" "$DB_USER" "$DB_PASSWORD_CAR2"
}
...
insert_documents() {
  ...
  if [ "$response" -eq 201 ]; then
    echo "Car bulk documents added successfully to database '$COUCHDB_DB_NAME'."
  else
